...
export const AnimatedThemeToggler = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    const applyTheme = () => {
      ...
      if (isCurrentlyDark) {
        ...
        localStorage.setItem('mergex-detail-theme', 'light');
        setIsDark(false);
      } else {
        document.documentElement.classList.add('dark');
        localStorage.setItem('mergex-detail-theme', 'dark');
        setIsDark(true);
      }
      // Dispatch a custom event so other components (like comments or charts) can sync with theme changes.
      window.dispatchEvent(new CustomEvent('mergex:theme-change', { detail: { theme: isCurrentlyDark ? 'light' : 'dark' } }));
    };
  ...
  if (!mounted) {
    return (
      <button
        type="button"
        className={cn(
          'h-8 w-8 text-white/50 hover:text-white flex items-center justify-center rounded-md hover:bg-white/10 transition-colors',
          className
        )}
        {...props}
      >
  ...
  return (
    <button
      ...
      onClick={toggleTheme}
      className={cn(
        'h-8 w-8 text-white/50 hover:text-white flex items-center justify-center rounded-md hover:bg-white/10 transition-colors cursor-pointer focus:outline-none',
        className
      )}
      {...props}
    >
